...
def initialize(model, parameters, num_nodes: int = 0) -> Tuple[int, DemographicsStatic, np.ndarray, np.ndarray]:
    """Initialize the model with the London scenario."""

    data = sc.load(Path(__file__).parent / "data" / "londondata.sc")

    # get distances between nodes
    distances = np.load(Path(__file__).parent /  "data" / "londondist.npy")
